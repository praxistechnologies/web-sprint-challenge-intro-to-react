...
A side effect is anything that affects something outside the scope of the function being executed. Fetching data from an API, timers, logging, and manually manipulating the DOM are all examples of side effects. A side effect is something that can cause a component to return a different output for the same state and props. To initiate a side effect, one can the useEffect() hook that tells React that a component needs to run, or execute, some side effect. Sync enters in as we don't want to run a whole all the hooks every time there is a change of state or prop. Sync allows for side effects in and with state and prop changes to do something after the page render. Effects scheduled with useEffect don’t block the browser from updating the screen and React guarantees the DOM has been updated by the time it runs the effects.